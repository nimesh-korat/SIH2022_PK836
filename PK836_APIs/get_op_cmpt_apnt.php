...
if (isset($_POST["OP_ID"])) {
    ...
    $checkQuery = "SELECT login_table.F_NAME, login_table.L_NAME, appointment_details.* FROM login_table LEFT JOIN appointment_details ON login_table.USERID = appointment_details.USERID LEFT JOIN op_fetch_appt ON op_fetch_appt.APPT_ID=appointment_details.APNT_ID WHERE op_fetch_appt.APPT_STATUS= 'COMPLETED' AND op_fetch_appt.OP_ID = '$OP_ID'"; // change here.
    ...
    if ($result->num_rows == 0) {
        $response["error"] = TRUE;
        $response["message"] = "Sorry no completed appointment found.";
        echo json_encode($response);
        exit;
    } else {
