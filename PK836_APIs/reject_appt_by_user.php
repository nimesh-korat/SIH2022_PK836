...
if (isset($_POST["APPT_ID"]) && isset($_POST["USERID"])) {
	...
	if ($checkAPNTresult->num_rows == 0) {
		...
		$userRjectQuery = "INSERT INTO rejected_appt (`APPT_ID`, `RJCTD_BY`,`RJCT_OP_USER_ID`) VALUES ('$APPT_ID', 'USER','$USERID')";
		$userRjectResult = mysqli_query($db_con, $userRjectQuery);

		$UPDATEQuery = "UPDATE `appointment_details` SET `APNT_STATUS` = 'CANCELLED_BY_USER' WHERE `APNT_ID` = '$APPT_ID'";
		$UPDATEResult = mysqli_query($db_con, $UPDATEQuery);
		if ($userRjectResult && $UPDATEResult) {
